...
/////function to get current service id and sub-service id from html url after ?
function get_curr_serv_subserv_fromUrl(){
	var serv_subserv_arr=["",""];
	var url_string = window.location.href; //
	var url = new URL(url_string);
	var mainserviceid = url.searchParams.get("mainserviceID");
	serv_subserv_arr[0]=mainserviceid
	var subserviceid = url.searchParams.get("subserviceid");
	serv_subserv_arr[1]=subserviceid
	return serv_subserv_arr
}
...
function set_page_title_forsubserv(){
	...
	var mainsubserv=get_curr_serv_subserv_fromUrl();///main service
		var mainserviceid = mainsubserv[0];
		var titleofpage="";
		if(mainserviceid>0){
				var mainservicObj=DataClass.get_service_obj_byid(mainserviceid,DataClass.DataJSON.data.allservices);
				titleofpage=mainservicObj[0].title
			}else{
...
function startloadingtheapp(){
	...
	///////////////Get the main Service ID from the url////////////////////////////////////////
		...
		var all_sub_services=DataClass.get_service_obj_byid(mainserviceid,DataClass.DataJSON.data.allservices);
		all_sub_services=all_sub_services[0].sub_services
		...
		for(i=0;i<all_sub_services.length;i++){
			...
			//console.log(service_has_subservice)
			...
			if(service_has_subservice==0 ){
				...
				var condslenth=0; //var condslenth=service_obj.conds.length
				var hideenterkey=0
